...
Ray Camera::generate_ray_for_thin_lens(double x, double y, double rndR, double rndTheta) const {
	...
	// From left to right: image plane (1 z unit left of camera), camera (z = 0), sensor plane (1 z unit right of camera), focal plane (focalDistance z units right of camera)
	double x_sensor_cam = (x - 0.5) * 2.0 * tan(radians(hFov) * 0.5);
	double y_sensor_cam = (y - 0.5) * 2.0 * tan(radians(vFov) * 0.5);
	Vector3D pSensor_cam = Vector3D(x_sensor_cam, y_sensor_cam, -1.0); // location of sensor plane
	//Vector3D pSensor_world = c2w * pSensor_cam;
	//Vector3D dirRed = pSensor_world.unit();
	//Ray rRed = Ray(pos, dirRed);

	Vector3D pFocus_cam = pSensor_cam * focalDistance;
	Vector3D pFocus_world = c2w * pFocus_cam;

	Vector3D pLens_cam = Vector3D(lensRadius*sqrt(rndR)*cos(rndTheta), lensRadius*sqrt(rndR)*sin(rndTheta), 0.0);
	Vector3D pLens_world = c2w * pLens_cam;
	//pLens_world += pos;

	//Vector3D vFocus_cam = Vector3D(1.0, 0.0, -focalDistance); // arbitrary point on focal plane
	//Vector3D vFocus_world = c2w * vFocus_cam;
	//Vector3D nFocus_cam = Vector3D(0.0, 0.0, 1.0); // normal vector of focal plane
	//Vector3D nFocus_world = c2w * nFocus_cam;
	////double t = dot(vFocus_world - rRed.o, nFocus_world) / dot(rRed.d, nFocus_world);
	//double t = (vFocus_world.z - rRed.o.z) / rRed.d.z;
	//if (t < 0) {
	//	std::cout << t << std::endl;
	//	return Ray(Vector3D(), Vector3D());
	//}
	//Vector3D pFocus_world = rRed.at_time(t);

	//Vector3D dirBlue = (pFocus_world - pLens_world).unit();

	Vector3D dirBlue = (pFocus_world - pLens_world).unit();
	//dirBlue = c2w * dirBlue;

	Ray rBlue = Ray(pLens_world + pos, dirBlue);
	rBlue.min_t = nClip;
	rBlue.max_t = fClip;
